...
########################################
#Generate Word2Vec model for the corpus#
########################################

y = nltk.sent_tokenize(gutenberg.raw(fileids = guten_train))
x = [gensim.utils.simple_preprocess(y) for y in y]
#model = gensim.models.Word2Vec(x, size=200, window=5, min_count=1, workers=4)
z = [z for y in x for z in y]
...
########################################
#Generate Word2Vec model for the Test corpus#
########################################

y = nltk.sent_tokenize(gutenberg.raw(fileids = guten_test))
x = [gensim.utils.simple_preprocess(y) for y in y]
#model = gensim.models.Word2Vec(x, size=200, window=5, min_count=1, workers=4)
z_1 = [z for y in x for z in y]


# In[86]:


y = nltk.sent_tokenize(gutenberg.raw())
x = [gensim.utils.simple_preprocess(y) for y in y]
z_2 = [z for y in x for z in y]
lookup_1 = nltk.FreqDist(z_2)
...
###########################################
lookup= []
for p in lookup_1.keys():
    lookup.append(p)
numer_list = []
for step, k in enumerate(z):
...
###########################################
#Create word Embedding list for Test Corpus#
###########################################
lookup= []
for p in lookup_1.keys():
    lookup.append(p)
numer_list_1 = []
for step, k in enumerate(z_1):
...
##########################################################################################################################
def run_epoch(session, m, data, data_1, eval_op, verbose=False):
...
with tf.Graph().as_default():
    ...
    for i in range(max_max_epoch):
        ...
        train_perplexity, state = run_epoch(session, m, train_data, test_data, m.train_op, verbose=True)
